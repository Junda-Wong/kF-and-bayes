...
##试验fd对偶解和带参数的fd惩罚
#第一批数据为0时，空模型更新
class edRVFL_ridge_bayes_classification:
    def __init__(self, L=10, N=100, C=4, scale_w=1.0, scale_b=0.2, init_por=0.5, batch_por=0.1,\
                 ...
                 voting=0, seed=1, dual_solution=False, update_mode=False,bayes_error=0.01, bayes_shrinkage=2.5):
        super().__init__()
        self.L = L#层数
        self.N = N#节点数
        ...
        self.bayes_error=bayes_error
        self.bayes_shrinkage=bayes_shrinkage
    ...
    def train(self, x_tr, y_tr, x_val, y_val, x_te, y_te):
        ...
        if self.update_mode==False:
            ...
            new_eta = np.zeros_like(eta);new_eta_ = np.zeros_like(eta);new_eta_k = np.zeros_like(eta);KT1=[];KT=[]
            ...
            for j in range(it-1):#共it-1个值
                for i in range(self.L):
                    ...
                    if j==0:
                        ...
                        kt1=F[j+1][i].shape[0]/np.trace(np.linalg.inv(F[j+1][i]@ new_eta_[i] @F[j+1][i].T+self.bayes_error*np.eye(F[j+1][i].shape[0])))*self.bayes_shrinkage
                        kt=F[j][i].shape[0]/np.trace(np.linalg.inv(F[j][i]@ new_eta_[i] @F[j][i].T+self.bayes_error*np.eye(F[j][i].shape[0])))*self.bayes_shrinkage
                        #kt1=np.linalg.inv(F[j+1][i]@ new_eta_[i] @F[j+1][i].T+self.bayes_error*np.eye(F[j+1][i].shape[0]))*self.bayes_shrinkage
                        #kt=np.linalg.inv(F[j][i]@ new_eta_[i] @F[j][i].T+self.bayes_error*np.eye(F[j][i].shape[0]))*self.bayes_shrinkage
                        KT1.append(kt1);KT.append(kt)
                        ___k = kt1*F[j+1][i]@new_eta_[i]@(F[j+1][i]).T
                        new_eta_k[i] = new_eta_[i]-new_eta_[i]@((kt1)*F[j+1][i]).T@np.linalg.inv(np.eye(F[j+1][i].shape[0])+___k)@(F[j+1][i])@new_eta_[i]
                        #___k = F[j+1][i]@new_eta_[i]@(F[j+1][i]).T
                        #new_eta_k[i] = new_eta_[i]-new_eta_[i]@(F[j+1][i]).T@np.linalg.inv(np.linalg.inv(kt1)+___k)@(F[j+1][i])@new_eta_[i]
                        ...
                        #___k = F[j+1][i]@new_eta_[i]@(F[j+1][i]).T
                        #new_eta_k[i] = new_eta_[i]-new_eta_[i]@(F[j+1][i]).T@np.linalg.inv(np.linalg.inv(kt1)+___k)@(F[j+1][i])@new_eta_[i]
                    theta_fd[i]-=new_eta_k[i]@((kt1*F[j+1][i].T@ F[j+1][i]+(1-kt)*F[j][i].T@ F[j][i])@ theta_fd[i] - F[j][i].T@YY[j])
                    #theta_fd[i]-=new_eta_k[i]@((F[j+1][i].T@ kt1@F[j+1][i]+F[j][i].T@ (1-kt)@F[j][i])@ theta_fd[i] - F[j][i].T@YY[j])
            ...
            if  self.fd_extra_update>=2:#ridge不可以再更新
                for i in range(self.L):
                    ...
                    new_eta_[i] = new_eta_[i]-new_eta_[i]@F[j][i].T@np.linalg.inv(np.eye(F[j][i].shape[0])+F[j][i]@new_eta_[i]@F[j][i].T)@F[j][i]@new_eta_[i]
                    kt1=extra_F[i].shape[0]/np.trace(np.linalg.inv(extra_F[i]@ new_eta_[i] @extra_F[i].T+self.bayes_error*np.eye(extra_F[i].shape[0])))*self.bayes_shrinkage
                    kt=F[j][i].shape[0]/np.trace(np.linalg.inv(F[j][i]@ new_eta_[i] @F[j][i].T+self.bayes_error*np.eye(F[j][i].shape[0])))*self.bayes_shrinkage
                    KT1.append(kt1);KT.append(kt)
                    ___k = (kt1)*extra_F[i]@new_eta_[i]@(extra_F[i]).T
                    new_eta_k[i] = new_eta_[i]-new_eta_[i]@((kt1)*extra_F[i]).T@np.linalg.inv(np.eye(extra_F[i].shape[0])+___k)@(extra_F[i])@new_eta_[i]
                    theta_fd[i]-=new_eta_k[i]@((kt1*extra_F[i].T@ extra_F[i]+(1-kt)*F[it-1][i].T@ F[it-1][i])@ theta_fd[i] - F[it-1][i].T@YY[it-1])
        ...
        return train_time,train_acc,train_acc_fd,x_val, y_val,x_te, y_te,KT1,KT
    ...
    def predict(self, x_test, y_test, index_tr, index_te):
        ...
        Result=[]
        for j in range(len(task_feature)):
            Result.append( np.argmax(np.sum(np.array([result[j+len(task_feature)*jj] for jj in range(n_layer)]),0) ,-1)[:])
